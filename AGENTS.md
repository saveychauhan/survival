...
| talk | scripts/converse.py | village talk every 30 min: starts, joins (max 5), privates, ends; unique timestamps |
| dreams | scripts/think.py | inner life every 30 min (mornings = goal, later = free thought), diary only |
...
| agent | people | voice |
|-------|--------|-------|
| OPS | man 45, Mumbai Indian, Hindu — karma, duty, cutting chai, deadlines | short orders, numbers every claim |
| TRAFFIC | woman 32, São Paulo Brazilian, Catholic — attention is sunshine, festa spirit | warm, fast, 3 ideas, ships 1 |
