...
var TW=36,TH=18,OX=cv.width/2,OY=96,nightTarget=0,nightF=0,paused=false,T=0;
...
var trees=[],flowers=[];
(function(){var banned=[[0,0,6,13],[9,2,12,5],[9,7,12,10],[9,12,12,15]];
for(var i=0;i<26;i++){var x=Math.floor(rnd()*15),y=Math.floor(rnd()*15),bad=false;
...
ctx.fillStyle='#fff';ctx.font='10px sans-serif';ctx.textAlign='center';ctx.fillText(name,(p[0]+q[0])/2,p[1]-56)}
...
ctx.fillStyle='#fff';ctx.font='10px sans-serif';ctx.textAlign='center';ctx.fillText('SQUARE',c[0],c[1]+34)}
...
ctx.fillStyle='#fff';ctx.font='10px sans-serif';ctx.textAlign='center';ctx.fillText('ARENA',c[0],c[1]+36)}
...
for(var i=0;i<26;i++)flies.push({x:Math.random()*15,y:Math.random()*15,ph:Math.random()*6});
var HOMES=[['OPS',1,1],['TRAFFIC',1,4],['MONETIZE',1,7],['SCOUT',1,10],['G-M',4,1],['CRI',4,4],['W01',4,7],['W02',4,10]];
var HCOLS=['#f59e0b','#4ade80','#38bdf8','#c084fc','#fb7185','#f87171','#a5b4fc','#a5b4fc'];
var winState={};
function step(dt){
T+=dt;
clouds.forEach(function(c){c.x+=c.v*dt;if(c.x>860)c.x=-80});
if(Math.random()<dt*6&&smoke.length<50){var h=HOMES[Math.floor(Math.random()*HOMES.length)];smoke.push({x:h[1]+0.72,y:h[2]+0.3,r:2,a:0.5})}
...
var PL={SQUARE:[10.5,3.5],MARKET:[10.5,8.5],ARENA:[10.5,13.2]};
var walkers=DATA.souls.map(function(s,i){
var hx=1+(i%2)*3,hy=1+Math.floor(i/2)*2.6;if(hy>12)hy=11-(i%2);
...
ctx.font='10px sans-serif';var label=w.s.short,tw=ctx.measureText(label).width;
...
for(var x=0;x<15;x++)for(var y=0;y<15;y++)tile(x,y);
// paths
for(var y=1;y<=13;y++){path(6,y)}for(var x=6;x<=10;x++){path(x,3);path(x,8);path(x,13)}
[[6,3],[6,8],[6,13]].forEach(function(p){var q=px(p[0],p[1]);ctx.fillStyle=mix('#6b543a','#2a2419',nightF);ctx.beginPath();ctx.ellipse(q[0],q[1],16,8,0,0,7);ctx.fill()});
fountain(10.5,3.5);
stall(9.3,8.2,'#ef4444');stall(10.1,8.5,'#f59e0b');stall(10.9,8.2,'#22c55e');
var m=px(10.4,9.4);ctx.fillStyle='#fff';ctx.font='10px sans-serif';ctx.textAlign='center';ctx.fillText('MARKET',m[0],m[1]+26);
arena(10.5,13.2);
torch(9.2,12.2);torch(11.8,12.2);torch(9.2,14.4);torch(11.8,14.4);
trees.forEach(function(t){pine(t[0],t[1],t[2])});
flowers.forEach(function(f){flower(f[0],f[1],f[2])});
// houses + smoke sources
var i=0;HOMES.forEach(function(h){hut(h[1],h[2],HCOLS[i%HCOLS.length],h[0],winState);i++});
// smoke
...
// fireflies
if(nightF>0.05)flies.forEach(function(f,i){var p=px((f.x+T*0.3+i)%15,(f.y+Math.sin(T+i)*0.5+15)%15);ctx.fillStyle='rgba(253,224,71,'+(nightF*(0.4+0.4*Math.sin(T*2+f.ph)))+')';ctx.beginPath();ctx.arc(p[0],p[1]-14,2,0,7);ctx.fill()});
// walkers sorted
